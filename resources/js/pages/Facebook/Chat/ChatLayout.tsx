                ...
                {selectedConversation ? (
                    <>
                        ...
                        <div className="border-b dark:border-neutral-800 bg-white dark:bg-neutral-900 px-6 py-4 shadow-sm">
                            ...
                            {selectedConversation.last_message_at && 
                             new Date().getTime() - new Date(selectedConversation.last_message_at).getTime() > 24 * 60 * 60 * 1000 && (
                                <div className="mt-3 px-3 py-2 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-lg flex items-start gap-2">
                                    <svg className="w-5 h-5 text-amber-600 dark:text-amber-400 flex-shrink-0 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
                                    </svg>
                                    <div className="flex-1">
                                        <p className="text-sm font-medium text-amber-800 dark:text-amber-300">Outside 24-hour window</p>
                                        <p className="text-xs text-amber-700 dark:text-amber-400 mt-1">
                                            Requires HUMAN_AGENT permission. Messages may fail if not approved.
                                        </p>
                                    </div>
                                </div>
                            )}
